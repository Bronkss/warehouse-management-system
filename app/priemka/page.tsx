...
function renderAcceptanceBarcodeSvg(barcodeFromDb: string): string {
    ...
    try {
        ...
        console.error('Ошибка формирования штрихкода:', error)
        return ''
    }
}
    ...
    const labels = products
        .map(product => {
            const price = new Intl.NumberFormat('ru-RU', {
                maximumFractionDigits: 0,
            }).format(Math.round(product.sellingPrice))

            const barcodeSvg = renderAcceptanceBarcodeSvg(product.barcode)
            const unit = product.unit === 'weight' ? 'за кг' : 'за шт'
...
function buildAcceptanceA4LabelsHtml(
    ...
    const sheets: AcceptanceLabelProduct[][] = []

    for (
        let index = 0;
        index < products.length;
        index += LABELS_PER_SHEET
    ) {
        sheets.push(products.slice(index, index + LABELS_PER_SHEET))
    }
    ...
    return `
        ...
        <head>
            <meta charset="utf-8" />
            <title>Ценники 58×40 мм — A4</title>
...
export default function Page() {
    ...
    const [isPriceLabelsModalOpen, setIsPriceLabelsModalOpen] = useState(false)
    const [acceptanceLabelProducts, setAcceptanceLabelProducts] = useState<AcceptanceLabelProduct[]>([])
    ...
    useEffect(() => {
        const hasOpenModal = isHistoryModalOpen || isAcceptanceModalOpen || isPriceLabelsModalOpen
    ...
    }, [isAcceptanceModalOpen, isHistoryModalOpen, isPriceLabelsModalOpen])
    ...
    const handleAcceptanceSaved = async (
        result: {
            acceptanceId?: number
        ...
        const labelProducts: AcceptanceLabelProduct[] = rows.map(item => ({
            id: item.product.id,
            name: item.product.name,
            category: item.category || item.product.category || '',
            barcode: item.product.barcode || '',
            sellingPrice: Math.round(
                toNumber(item.sellingPrice || item.product.sellingPrice)
            ),
        ...
        setAcceptanceLabelProducts(labelProducts)

        if (labelProducts.length > 0) {
            setIsPriceLabelsModalOpen(true)
        }
    }
    ...
    const updateAcceptanceLabelQuantity = (
        ...
        setAcceptanceLabelProducts(prev =>
            prev.map(product =>
    ...
    const toggleAcceptanceLabelProduct = (productId: number) => {
        setAcceptanceLabelProducts(prev =>
            prev.map(product =>
                product.id === productId
                    ? {
    ...
    const getProductsForAcceptanceLabels = (): AcceptanceLabelProduct[] => {
        return acceptanceLabelProducts
    ...
    const printAcceptancePriceLabels = (
        html: string,
        layout: 'a4' | 'thermal'
    ) => {
    ...
    const handlePrintAcceptanceLabels = () => {
        ...
        const productsForPrint = getProductsForAcceptanceLabels()

        if (productsForPrint.length === 0) {
            setError('Выберите хотя бы один товар для печати ценников')
            return
        }

        try {
            printAcceptancePriceLabels(
                buildAcceptanceThermalLabelsHtml(productsForPrint),
                'thermal'
            )

            setIsPriceLabelsModalOpen(false)
            setAcceptanceLabelProducts([])
        } catch (error) {
            console.error('Ошибка печати XPrinter:', error)
            setError(
    ...
    const handlePrintAcceptanceLabelsA4 = () => {
        ...
        try {
            printAcceptancePriceLabels(
                buildAcceptanceA4LabelsHtml(productsForPrint),
                'a4'
            )
            ...
            console.error('Ошибка печати A4:', error)
            setError(
                error instanceof Error
                    ? error.message
                    : 'Не удалось сформировать ценники A4'
            )
        }
    }
            ...
            <section className="relative w-full min-h-screen bg-gray-50 p-4">
                <div className="mx-auto max-w-[1700px] space-y-4">
                    ...
                    <ProductMovementForm
                        ...
                        comment={comment}
                        onAcceptanceSaved={handleAcceptanceSaved}
                    />
                ...
                {isPriceLabelsModalOpen && (
                    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 px-3 py-4">
                            ...
                            <div className="border-b border-gray-100 px-6 py-5">
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-gray-500">
                                            Ниже находятся товары из только что созданной приёмки.
                                            Выберите нужные позиции и количество ценников.
                                        </p>
                                    </div>
                            ...
                            <div className="min-h-0 flex-1 overflow-y-auto p-5">
                                <div className="overflow-hidden rounded-xl border border-gray-200">
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                        {acceptanceLabelProducts.map(product => (
                                            <tr
                                                key={product.id}
                                            ...
                                            >
                                                <td className="p-3 text-center">
                                                    <input
                                                        ...
                                                        onChange={() =>
                                                            toggleAcceptanceLabelProduct(product.id)
                                                        }
                                                        className="h-4 w-4"
                                                    />
                                                </td>
                        ...
                        {acceptanceLabelProducts.filter(item => item.selected).length}
                    </span>
                        ...
                        {acceptanceLabelProducts.reduce(
                            (sum, item) =>
                                item.selected ? sum + item.quantity : sum,
                            0
                        )}
                    </span>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <button
                                        type="button"
                                        onClick={handlePrintAcceptanceLabelsA4}
                                        disabled={
                                            !acceptanceLabelProducts.some(item => item.selected)
                                        }
                                    ...
                                    <button
                                        type="button"
                                        onClick={handlePrintAcceptanceLabels}
                                        disabled={
                                            !acceptanceLabelProducts.some(item => item.selected)
                                        }
